...
def get_account_mylist(account_id):
    ...
    elif account_mylist_response.status != const.HTTP_RETURN_CODE_SUCCEED:
...
def get_one_page_account_video(account_id, page_count):
    ...
    if video_index_response.status == 404:
        raise crawler.CrawlerException("账号不存在")
    elif video_index_response.status != const.HTTP_RETURN_CODE_SUCCEED:
...
def get_one_page_mylist_video(list_id, page_count):
    ...
    elif mylist_pagination_response.status != const.HTTP_RETURN_CODE_SUCCEED:
