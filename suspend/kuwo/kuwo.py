...
def get_one_page_playlist(playlist_id, page_count):
    ...
    playlist_pagination_response = net.request(playlist_pagination_url, method="GET", fields=query_data, cookies={"kw_token": CSRF_TOKEN}, headers={"csrf": CSRF_TOKEN}, json_decode=True)
    result = {
...
def get_audio_info_page(audio_id):
    ...
    audio_info_response = net.request(audio_info_url, method="GET", fields=query_data, cookies={"kw_token": CSRF_TOKEN}, headers={"csrf": CSRF_TOKEN}, json_decode=True)
    result = {
        "audio_url": "",  # 音频地址
    }
