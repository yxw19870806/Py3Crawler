...
class KuWoPlaylist(kuwo.KuWo):
    ...
    def get_playlist_id_from_console():
        playlist_url = input(tool.convert_timestamp_to_formatted_time() + " 请输入歌单地址：").lower()
        playlist_id = None
    ...
    def _main(self):
        ...
        while len(audio_info_list) > 0:
            ...
            file_path = os.path.join(dir_path, "%s - %s.%s" % (audio_info["audio_id"], path.filter_text(audio_info["audio_title"]), net.get_url_file_ext(audio_info_response["audio_url"])))
