...
class Download(crawler.DownloadThread):
    ...
    def get_crawl_list(self):
        ...
        self.trace("首页解析的全部音频：%s" % audit_pagination_response["audio_info_list"])
        self.step("首页解析获取%s首音频" % len(audit_pagination_response["audio_info_list"]))
    ...
    def crawl_audio(self, audio_info):
        ...
        file_path = os.path.join(self.main_thread.audio_download_path, self.display_name, "%07d - %s.%s" % (audio_info["audio_id"], path.filter_text(audio_info["audio_title"]), net.get_file_extension(audio_url)))
