...
def main():
    # 初始化类
    prpr_class = prpr.PrPr(extra_sys_config={crawler.SYS_NOT_CHECK_SAVE_DATA: True})

    account_list_from_api = get_account_list_from_api()
    if len(account_list_from_api) > 0:
        # 存档位置
        for account_id in account_list_from_api:
            if account_id not in prpr_class.account_list:
                prpr_class.account_list[account_id] = [account_id, "", account_list_from_api[account_id]]
        temp_list = [prpr_class.account_list[key] for key in sorted(prpr_class.account_list.keys())]
        file.write_file(tool.list_to_string(temp_list), prpr_class.save_data_path, file.WRITE_FILE_TYPE_REPLACE)
