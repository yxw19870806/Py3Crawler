...
def get_album_page(album_url):
    ...
    # 获取图集图片地址
    photo_list_selector = pq(album_response_content).find(".post-content .photoThum a")
    if photo_list_selector.length == 0:
        raise crawler.CrawlerException("页面截取图片地址失败\n%s" % album_response_content)
    for photo_index in range(1, photo_list_selector.length):
        result["photo_url_list"].append(photo_list_selector.eq(photo_index).attr("href"))
    return result
            ...
            while len(album_info_list) > 0:
                ...
                for photo_url in album_response["photo_url_list"]:
                    ...
                    if save_file_return["status"] == 1:
                        log.step("图集%s《%s》 第%s张图片下载成功" % (album_info["album_id"], album_info["album_title"], photo_index))
                    else:
                        log.error("图集%s《%s》 %s 第%s张图片 %s 下载失败，原因：%s" % (album_info["album_id"], album_info["album_title"], album_info["album_url"], photo_index, photo_url, crawler.download_failre(save_file_return["code"])))
                    photo_index += 1
                # 图集内图片全部下载完毕
                temp_path = ""  # 临时目录设置清除
