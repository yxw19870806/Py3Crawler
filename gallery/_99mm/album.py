...
def get_album_page(album_id):
    ...
    if not js_string:
        js_string = tool.find_sub_string(album_pagination_response_content, "var iaStr = ", ";</script>")
        if not js_string:
            raise crawler.CrawlerException("%s 页面截取页面参数'iaStr'失败\n%s" % (album_pagination_url, album_pagination_response_content))
        temp_js_list = tool.json_decode(js_string)
        if temp_js_list is None:
            raise crawler.CrawlerException("%s 页面加载参数'iaStr'失败\n%s" % (album_pagination_url, album_pagination_response_content))
        var_list = ["", "", temp_js_list[2], "", temp_js_list[1].split("/")[0], "", "", ""] + temp_js_list[6].split(",")
    else:
...
class Download(crawler.DownloadThread):
    ...
    def run(self):
        self.result = net.save_net_file(self.photo_url, self.file_path, header_list={"Referer": "http://www.99mm.me/"})
