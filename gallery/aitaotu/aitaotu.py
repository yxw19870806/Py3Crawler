...
def get_album_page(album_id):
    ...
    while page_count <= max_page_count:
        ...
        if album_pagination_response_content.find("[1040] Too many connectionsError!: Db Error!") >= 0:
            has_error = True
            page_count += 1
            continue
        if not result["album_title"]:
            ...
            if pagination_selector.length == 0:
                ...
                max_page_count = tool.find_sub_string(last_pagination_url, "/%s_" % album_id, ".html")
                if not crawler.is_integer(max_page_count):
                    raise crawler.CrawlerException("最后页地址截取最后页失败\n%s" % album_pagination_response_content)
                max_page_count = int(max_page_count)
        # 获取图集图片地址
        photo_list_selector = pq(album_pagination_response_content).find(".big-pic a img")
        ...
        photo_count_each_page = max(photo_count_each_page, photo_list_selector.length)
...
class Download(crawler.DownloadThread):
    ...
    def run(self):
        self.result = net.save_net_file(self.photo_url, self.file_path, header_list={"Referer": "https://www.aitaotu.com/"})
