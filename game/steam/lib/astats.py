...
如有问题或建议请联系
"""
from common import output, net, crawler, tool
...
def get_game_invalid_achievements(game_id):
    ...
    if game_index_response.status != net.HTTP_RETURN_CODE_SUCCEED:
        output.print_msg(f"游戏 {game_id} 访问失败")
        tool.process_exit()
    # game id 不存在
    if game_index_response.data.decode(errors="ignore").find("This game cannot be found in the database.") >= 0:
        return
    ...
    if not tool.is_integer(achievement_text):
        ...
        if invalid_achievement_text:
            output.print_msg(f"游戏 {game_id} 存在无效成就：{invalid_achievement_text}")
        else:
            output.print_msg(f"游戏 {game_id}, 存在未知成就文字：{invalid_achievement_text}")
