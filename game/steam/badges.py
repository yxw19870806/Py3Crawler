...
def main():
    ...
    MIN_CARD_PRICE = crawler.analysis_config(config, "MIN_CARD_PRICE", 0, enum.ConfigAnalysisMode.BOOLEAN)
    MAX_CARD_PRICE = crawler.analysis_config(config, "MAX_CARD_PRICE", 0.5, enum.ConfigAnalysisMode.BOOLEAN)
    MAX_TOTAL_PRICE = crawler.analysis_config(config, "MAX_TOTAL_PRICE", 1.5, enum.ConfigAnalysisMode.BOOLEAN)
    IS_TOTAL_CARD = crawler.analysis_config(config, "IS_TOTAL_CARD", False, enum.ConfigAnalysisMode.BOOLEAN)
    ...
    for badges_detail_url in badges_detail_url_list:
        ...
        if len(wanted_card_list) == 0:
            ...
            file.write_file(tool.json_encode(skip_list), skip_list_file_path, enum.WriteFileMode.REPLACE)
            continue
