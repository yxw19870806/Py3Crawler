...
def main():
    # 初始化类
    youtube_class = youtube.Youtube(extra_sys_config={enum.SysConfigKey.NOT_CHECK_SAVE_DATA: True})
    niconico_class = niconico.NicoNico(extra_sys_config={enum.SysConfigKey.NOT_CHECK_SAVE_DATA: True})
    ...
    for account_id in account_id_list["niconico"]:
        ...
        file.write_file(tool.json_encode(niconico_mylist_list), niconico_mylist_cache_path, enum.WriteFileMode.REPLACE)
    ...
    file.write_file(tool.list_to_string(youtube_class.save_data.values()), youtube_class.save_data_path, enum.WriteFileMode.REPLACE)
    ...
    file.write_file(tool.list_to_string(niconico_class.save_data.values()), niconico_class.save_data_path, enum.WriteFileMode.REPLACE)
