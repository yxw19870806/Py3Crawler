...
class Download(crawler.DownloadThread):
    ...
    def crawl_comic(self, chapter_info):
        ...
        # 图片下载
        photo_index = 1
        chapter_path = os.path.join(self.main_thread.photo_download_path, self.display_name, chapter_info["group_name"], "%06d %s" % (chapter_info["chapter_id"], chapter_info["chapter_name"]))
        # 设置临时目录
        ...
        for photo_url in chapter_response["photo_url_list"]:
            ...
            save_file_return = net.save_net_file(photo_url, photo_file_path, header_list={"Referer": "https://www.manhuagui.com/comic/%s/%s.html" % (self.comic_id, chapter_info["chapter_id"])})
