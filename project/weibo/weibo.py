...
def get_one_page_video(account_id, since_id, retry_count=0):
    ...
    for video_info in crawler.get_json_value(video_pagination_response.json_data, "data", "list", type_check=list):
        ...
        if page_type in [2, 5, 11]:
            ...
            raise CrawlerException(f"未知信息类型{page_type}")
        # 获取视频地址
        max_resolution = 0
        video_url = ""
        for single_video_info in video_detail_info_list:
            ...
            if video_type == 1:
                ...
                raise CrawlerException(f"未知视频类型{video_type}")
        if not video_url:
            raise CrawlerException(f"媒体信息{video_info}获取最大分辨率视频地址失败")
        result_video_info_list["video_url"] = video_url
...
class Weibo(crawler.Crawler):
    def __init__(self, **kwargs):
        ...
        sys_config = {
            ...
            const.SysConfigKey.SAVE_DATA_FORMATE: (0, ["", "0", "0"]),  # account_id  last_photo_id  last_video_id  (account_name)
        }
...
class CrawlerThread(crawler.CrawlerThread):
    ...
    def get_crawl_video_list(self):
        ...
        # 获取全部还未下载过需要解析的视频
        while not is_over:
            video_pagination_description = f"since_id：{since_id}后一页视频"
    ...
    def crawl_photo(self, photo_info):
        photo_name = f"%16d.{url.get_file_ext(photo_info['photo_url'], 'jpg')}" % photo_info["photo_id"]
    ...
    def photo_download_success_callback(self, photo_url, photo_path, photo_description, download_return: net.Download):
        if check_photo_invalid(photo_path):
            ...
            self.error(f"{photo_description} {photo_url} 资源已被限制，跳过")
            return False
    ...
    def _run(self):
        ...
        if self.main_thread.is_download_video:
            ...
            self.info(f"需要下载的全部视频片解析完毕，共{len(video_info_list)}个")
