    ...
    if os.path.exists(token_file_path):
        ...
        if tool.check_dict_sub_key(("api_key", "api_secret"), api_info):
            ...
            if get_access_token(api_info["api_key"], api_info["api_secret"]):
                log.info("access token get succeed!")
                return True
            else:
                log.info("api info has expired")
        else:
            log.info("decrypt api info failure")
        # token已经无效了，删除掉
        path.delete_dir_or_file(token_file_path)
    log.info("Please input api info")
    while True:
        ...
        if get_access_token(api_key, api_secret):
            ...
            log.info("access token get succeed!")
            return True
        log.info("incorrect api info, please type again!")
