...
def get_storage_file_count():
    if not os.path.exists(FILE_STORAGE_PATH):
        output.print_msg(f"file root path {FILE_STORAGE_PATH} not exist")
        return {}
    account_list = {}
    # 根目录下的全部子目录
    for dir_name in os.listdir(FILE_STORAGE_PATH):
        ...
        if IS_INTEGER:
            ...
            if file_max_index != total_file_count:
                output.print_msg(f"{dir_name} total count {total_file_count} not equal to max count {file_max_index}")
...
def get_save_data_file_count():
    if not os.path.exists(SAVE_DATA_FILE_PATH):
        output.print_msg(f"save data {SAVE_DATA_FILE_PATH} not exist")
        return {}
    account_list = {}
    for line in file.read_file(SAVE_DATA_FILE_PATH, file.READ_FILE_TYPE_LINE):
...
def check_count():
    ...
    for account_id in dict(account_list_from_storage):
        ...
        if account_list_from_save_data[account_id] != account_list_from_storage[account_id]:
            output.print_msg(f"{account_id} count in save data: {account_list_from_save_data[account_id]}, in root path: {account_list_from_storage[account_id]}")
    for account_id in dict(account_list_from_storage):
        if account_id not in account_list_from_save_data:
            output.print_msg(f"{account_id} not found in save data")
