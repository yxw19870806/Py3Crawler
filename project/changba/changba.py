...
class Download(crawler.DownloadThread):
    ...
    def crawl_audio(self, audio_info):
        ...
        download_return = net.Download(audio_play_response["audio_url"], file_path)
        if download_return.status == net.Download.DOWNLOAD_SUCCEED:
            self.total_audio_count += 1  # 计数累加
            self.step(f"歌曲{audio_info['audio_key']}《{audio_info['audio_title']}》下载成功")
        else:
            self.error(f"歌曲{audio_info['audio_key']}《{audio_info['audio_title']}》 {audio_play_response['audio_url']} 下载失败，原因：{crawler.download_failre(download_return.code)}")
