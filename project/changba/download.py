...
def main():
    ...
    while True:
        ...
        if audio_url.lower().find("//changba.com/s/") > 0:
            video_id = audio_url.split("/")[-1].split("?")[0]
        else:
            log.step("错误的歌曲地址，正确的地址格式如：https://changba.com/s/LBdSlkRwmqApasSCCVp5VA")
            continue
        # 访问歌曲播放页
        try:
            audio_response = changba.get_audio_play_page(video_id)
        except crawler.CrawlerException as e:
            log.error("解析歌曲下载地址失败，原因：%s" % e.message)
            tool.process_exit()
        # 开始下载
        file_type = audio_response["audio_url"].split(".")[-1]
        file_path = os.path.abspath(os.path.join(DOWNLOAD_FILE_PATH, "%s - %s.%s" % (video_id, path.filter_text(audio_response["audio_title"]), file_type)))
