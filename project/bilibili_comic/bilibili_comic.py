...
def get_comic_index_page(comic_id):
    ...
    api_response = net.Request(api_url, method="POST", fields=post_data, cookies=COOKIES).enable_json_decode()
    result = {
...
def get_chapter_page(ep_id):
    ...
    token_api_response = net.Request(token_api_url, method="POST", fields=post_data).enable_json_decode()
    if api_response.status != const.ResponseCode.SUCCEED:
        raise CrawlerException("图片token获取，" + crawler.request_failre(api_response.status))
...
class BiliBiliComic(crawler.Crawler):
    ...
    def init(self):
        ...
        while True:
            input_str = input(tool.convert_timestamp_to_formatted_time() + " 没有检测到账号登录状态，可能无法解析需要登录才能查看的漫画，继续程序(C)ontinue？或者退出程序(E)xit？:")
...
class CrawlerThread(crawler.CrawlerThread):
    ...
    def crawl_comic(self, comic_info):
        ...
        # 图片下载
        photo_index = 1
        chapter_name = "%06d %s" % (comic_info["ep_id"], comic_info["ep_name"])
        ...
        for photo_url in chapter_response["photo_url_list"]:
            ...
            photo_description = f"漫画{comic_info['ep_id']} 《{comic_info['ep_name']}》第{photo_index}/{len(chapter_response['photo_url_list'])}张图片"
