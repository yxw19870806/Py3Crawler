...
def check_safe_search():
    ...
    if setting_response.status == const.HTTP_RETURN_CODE_SUCCEED:
...
def get_account_index_page(account_name):
    ...
    elif account_index_response.status != const.HTTP_RETURN_CODE_SUCCEED:
...
def get_one_page_photo(user_id, page_count, api_key, csrf, request_id):
    ...
    if photo_pagination_response.status != const.HTTP_RETURN_CODE_SUCCEED:
