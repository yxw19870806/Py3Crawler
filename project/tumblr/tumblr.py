...
def get_index_setting(account_id):
    ...
    if index_response.status == 301:
        ...
        raise crawler.CrawlerException("账号不存在")
    elif index_response.status != const.HTTP_RETURN_CODE_SUCCEED:
        raise crawler.CrawlerException(crawler.request_failre(index_response.status))
    return is_https, is_private
...
def get_one_page_post(account_id, page_count, is_https):
    ...
    # elif post_pagination_response.status in [503, 504, const.HTTP_RETURN_CODE_RETRY] and page_count > 1:
    #     # 服务器错误，跳过这页
    #     log.error("%s 第%s页日志无法访问，跳过" % (account_id, page_count))
    #     return result
    elif post_pagination_response.status != const.HTTP_RETURN_CODE_SUCCEED:
...
def get_post_page(post_url, post_id):
    ...
    if post_response.status == 404:
        ...
        return result
    elif post_response.status != const.HTTP_RETURN_CODE_SUCCEED:
